...
const inputBallDx = Number.parseInt(input.get('ballx') ?? '1');
const inputBallDY = Number.parseInt(input.get('bally') ?? '1');
const inputSpeed = Number.parseInt(input.get('speed') ?? '1');
const MAX_POINTS = Number.parseInt(input.get('points') ?? '3');
const PADDLE_SPEED = Number.parseInt(input.get('paddle') ?? '1');
...
const resize = () => {
  ...
  paddles[1].setAttribute(
    'style',
    `width: ${PADDLE_SCALED.width}px; height: ${PADDLE_SCALED.height}px; left: calc(100% - ${PADDLE_SCALED.width}px);`
  );
...
const frame = (timestamp) => {
  ...
  // eslint-disable-next-line sonarjs/cognitive-complexity -- would need a big refactoring at some point
  setTimeout(() => {
    ...
    while (frames-- > 0) {
      ...
      } else if (x < BALL_RADIUS + PADDLE_WIDTH && y > state.paddles[0].y && y < state.paddles[0].y + PADDLE_HEIGHT) {
        dx = -dx;
