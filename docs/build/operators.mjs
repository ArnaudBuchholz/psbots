...
const ref2 = JSON.parse(
  await readFile(new URL('../operators/ref2.json', import.meta.url), 'utf8')
);
...
for (const [name, label] of Object.entries(ref2)) {
  const tags = {
    '⚙️': ['repl'],
    '❓': ['unknown'],
    '❌': ['not-implemented'],
  ...
  if (definition) {
    markdown.push(
      `🏷️ ${ definition.labels.map(label => '[[' + label + ']]').join(' ') }`,
